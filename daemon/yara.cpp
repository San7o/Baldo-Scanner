...
void Yara::callback_compilation(int error_level, const char* file_name,
                                int line_number,
                                [[maybe_unused]] const YR_RULE* rule,
                                const char* message,
...
*/
int Yara::callback_scan([[maybe_unused]] YR_SCAN_CONTEXT* context,
                        int message,
                        void* message_data,
                        void* user_data)
{
  if (message == CALLBACK_MSG_RULE_MATCHING)
  {
    ...
    Logger::Log(Enums::LogLevel::INFO,
                "Matched rule: " + std::string(rule->identifier));
...
void Yara::Scan(YR_RULES* rules, std::string filePath, ScanReport* report)
{
  ...
  int ret = yr_rules_scan_file(rules, filePath.c_str(), 0,
                               callback_scan, (void*) data, 10);
