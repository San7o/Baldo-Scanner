...
void Kernel::Init()
{
  ...
  if (ret != SQLITE_OK)
  {
    Logger::Log(Enums::LogLevel::ERROR,
                "Can't open database: " + std::string(sqlite3_errmsg(connection)));
  ...
  if (Kernel::family_id < 0)
  {
    ...
    Logger::Log(Enums::LogLevel::WARN,
                "Perhaps the kernel module is not loaded?");
    nl_socket_free(Kernel::sk);
  }
  else
  {
...
void Kernel::send_ip_to_firewall(uint32_t ipv4, Enums::IpAction action)
{
  ...
  Logger::Log(Enums::LogLevel::DEBUG,
              "Sent IP " + std::to_string(ipv4) + " to kernel");
  return;
...
void *Kernel::thread_listen_kernel([[maybe_unused]] void* arg)
{
  sigset_t set;
  sigemptyset(&set);
  ...
  nl_socket_modify_cb(Kernel::sk, NL_CB_MSG_IN, NL_CB_CUSTOM,
                      kernel_msg_callback, NULL);
...
int Kernel::kernel_msg_callback(struct nl_msg *msg,
